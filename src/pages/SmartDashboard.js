...
export default function SmartDashboard() {
  ...
  const integratedSites = smartData.filter(
    (row) => row['Integrated with DMRC App']?.toLowerCase() === 'ok'
  );

  const pendingSites = smartData.filter(
    (row) => row['Integrated with DMRC App']?.toLowerCase() !== 'ok'
  );

  const dayKey = smartData.length
    ? Object.keys(smartData[0]).find((k) =>
        k.toLowerCase().includes('days since handover')
      )
    : null;

  const stationKey = smartData.length
    ? Object.keys(smartData[0]).find((k) => k.toLowerCase() === 'station')
    : null;
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-3xl max-h-[80vh] overflow-auto">
            ...
            <ul className="space-y-2">
              {sixtyDaySites.map((site, i) => (
                <li
                  key={i}
                  className="border rounded p-3 bg-blue-50 shadow-sm"
                >
                  <p className="text-sm">
                    <strong>Station:</strong> {site[stationKey] || '—'} <br />
                    <strong>Line:</strong> {site[lineKey] || '—'} <br />
      ...
      {miscData.length > 1 && (
        <div className="mt-12">
          <h2 className="text-xl font-bold text-gray-700 mb-4">
            Miscellaneous Parking Info
          </h2>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-4 rounded shadow text-sm">
              <h3 className="font-semibold text-gray-800 mb-2">
                Summary of Parking at Metro Stations
              </h3>
...
function StatCard({ label, value, onClick, clickable }) {
  return (
    <div
      className={`bg-white shadow-lg rounded-xl p-6 text-center border-l-4 border-blue-500 ${
